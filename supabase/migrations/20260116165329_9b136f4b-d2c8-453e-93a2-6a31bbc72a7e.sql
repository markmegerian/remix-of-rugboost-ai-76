...
ALTER TABLE public.inspections ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;

-- Drop existing policies on inspections
DROP POLICY IF EXISTS "Allow public insert" ON public.inspections;
DROP POLICY IF EXISTS "Allow public read" ON public.inspections;
