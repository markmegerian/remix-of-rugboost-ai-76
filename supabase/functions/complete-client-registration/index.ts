...
async function checkDbRateLimit(supabaseAdmin: any, identifier: string): Promise<{ allowed: boolean }> {
  const { data: allowed } = await supabaseAdmin.rpc('check_rate_limit', {
    p_identifier: identifier,
    p_action: 'client_registration',
    p_max_requests: 5,
    p_window_minutes: 5,
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Database-backed rate limiting (need admin client first)
    ...
    const rateCheck = await checkDbRateLimit(supabaseAdmin, `registration:${clientIp}`);
